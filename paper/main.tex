...
\caption{Bidirectional reasoning pipeline with shared knowledge graph.}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Competency coverage: graph reasoner vs keyword baseline.}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Latency reduction through preview caching.}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \addplot coordinates {(Generation Outcomes,0.947)};
        \addplot coordinates {(Generation Outcomes,0.045)};
        \addplot coordinates {(Generation Outcomes,0.008)};
        \legend{First Attempt Success,Success Within Retry Budget,Failures}
        \node[font=\scriptsize\bfseries,anchor=west,xshift=15pt] at (axis cs:Generation Outcomes,0.4735) {94.7\%};
        \node[font=\scriptsize\bfseries,anchor=west,xshift=15pt] at (axis cs:Generation Outcomes,0.9695) {4.5\%};
        \node[font=\scriptsize\bfseries,anchor=west,xshift=15pt] at (axis cs:Generation Outcomes,1.04) {0.8\%};
        \end{axis}
    \end{tikzpicture}
    \caption{Resume generation reliability with retry recovery.}
...
Table~\ref{tab:competency} presents competency coverage metrics across evaluation pairs. The knowledge graph matching algorithm achieves 84.3\% required competency recall, representing a 27.9\% improvement over keyword baseline (65.9\%) and 15.2\% improvement over embedding baseline (73.2\%). This gain reflects the algorithm's ability to recognize transferable skills and partial competency matches through graph traversal, whereas keyword systems reject candidates lacking exact terminology and embedding systems struggle with nuanced competency definitions. Optional competency precision reaches 78.6\%, maintaining low false-positive rates while capturing relevant non-required skills. Figure~\ref{fig:coverage-comparison} additionally reports optional recall (coverage breadth) and false-positive rates; lower is better for false-positive rate while higher is better for other metrics. The threshold-based partial credit mechanism contributes significantly; when disabled, recall drops to 76.1\%, confirming the value of accommodating near-matches rather than imposing binary presence checks.
